...
%This dissertation makes several key contributions, which are summarized as follows:
%\begin{enumerate}
	%\item \textbf{Security Improvements for ERC-20 Tokens}: A thorough evaluation of the \mwa on ERC-20 tokens is conducted, proposing two new solutions to mitigate the vulnerability. One solution fully adheres to the ERC-20 standard while effectively addressing the attack.
	
	%\item \textbf{Systematization of ERC-20 Vulnerabilities}: The research systematizes existing vulnerabilities specific to ERC-20 tokens and then proposes a new implementation, TokenHook, which enhances security features of ERC-20 tokens in both Vyper and Solidity.
	...
	%\item \textbf{Proposing a New Decentralized LVT Model}: A fully decentralized design for LVTs, LeverEdge, is proposed, utilizing a hybrid L1-L2 model on Ethereum. It addresses the identified shortcomings of existing LVTs and offers a reference model aimed at safeguarding user investments.
%\end{enumerate}
%
